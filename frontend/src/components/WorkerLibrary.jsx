...
function AvatarDisplay({ avatar, name, size = 40 }) {
  const isUrl = avatar && avatar.startsWith('/')
  if (isUrl) return (
    <img src={avatar} alt={name}
      style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid #e5e5ea' }} />
  )
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: '#f3e8ff', color: '#7c3aed',
...
export default function WorkerLibrary() {
  ...
  const load = async () => {
    ...
    setTemplates(tmpl); setSkills(sk); setTeams(tm)
  }

  useEffect(() => { load() }, [])

  const openNew = () => {
    setForm(EMPTY_FORM); setSelected(null); setMode('new')
    setAvatarPreview(null); setAvatarFile(null)
  }
  const openEdit = (t) => {
    setForm({ ...EMPTY_FORM, ...t, capabilities: t.capabilities || [], skills: t.skills || [] })
    setSelected(t); setMode('edit')
    setAvatarPreview(t.avatar?.startsWith('/') ? t.avatar : null)
    setAvatarFile(null)
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (mode === 'new') {
        ...
        await fetch(`/api/worker-library/${selected.id}`, {
  ...
  if (mode === 'new' || mode === 'edit') return (
    <div style={{ padding: 24, flex: 1, overflowY: 'auto', height: '100vh' }}>
      ...
      {/* Avatar upload */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, padding: 16, background: '#fafafa', borderRadius: 12, border: '1px solid #e5e5ea' }}>
        <div style={{ position: 'relative', cursor: 'pointer', flexShrink: 0 }} onClick={() => fileInputRef.current?.click()}>
          {avatarPreview
            ? <img src={avatarPreview} alt="preview" style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', border: '3px solid #7c3aed' }} />
            : <div style={{ width: 72, height: 72, borderRadius: '50%', background: '#ede9fe', color: '#7c3aed', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', border: '3px dashed #c4b5fd' }}>
                {form.avatar && !form.avatar.startsWith('/') ? form.avatar : '👤'}
              </div>
          }
          <div style={{ position: 'absolute', bottom: 0, right: 0, background: '#7c3aed', color: 'white', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', border: '2px solid white' }}>📷</div>
        </div>
        <div>
          <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 4 }}>รูป Profile</div>
          <div style={{ fontSize: '0.75rem', color: '#888', marginBottom: 8 }}>JPG, PNG, GIF, WEBP · ไม่เกิน 5MB</div>
          <button onClick={() => fileInputRef.current?.click()}
            style={{ background: '#ede9fe', color: '#7c3aed', border: '1px solid #c4b5fd', borderRadius: 8, padding: '5px 14px', fontSize: '0.8rem', cursor: 'pointer', fontWeight: 600 }}>
            🗂️ Browse...
          </button>
          {avatarFile && <span style={{ marginLeft: 8, fontSize: '0.75rem', color: '#059669' }}>✓ {avatarFile.name}</span>}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        ...
        <div>
          ...
          <input value={form.avatar?.startsWith('/') ? '' : (form.avatar || '')} onChange={e => setForm(p => ({ ...p, avatar: e.target.value }))} placeholder="เช่น 🧠 หรือ ม" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid #e5e5ea', fontSize: '0.9rem', boxSizing: 'border-box', marginTop: 4 }} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <button onClick={save} disabled={uploading} style={{ flex: 1, background: '#7c3aed', color: 'white', border: 'none', borderRadius: 10, padding: 10, fontWeight: 600, cursor: uploading ? 'not-allowed' : 'pointer', opacity: uploading ? 0.6 : 1 }}>
